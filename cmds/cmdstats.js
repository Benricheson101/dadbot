  ...
  exec: (client, msg, args) => {
    ...
    function getData() {
      return new Promise((resolve, reject) => {
        // grafana.remoteEval(i, "let stats = require('./functions/commandStatistics');JSON.stringify(stats.uses.toJSON());").then(res => {
        //     let stuff = JSON.parse(res);
        //     for (let ii = 0; ii < stuff.length; ii++) {
        //         if (data.find(e => e[0] === stuff[ii][0])) {
        //             data[data.indexOf(data.find(e => e[0] === stuff[ii][0]))][1] += stuff[ii][1];
        //         } else {
        //             data.push(stuff[ii]);
        //         }
        //     }
